...
class BroadcastedLinear(nn.Module):
    ...
    def __init__(self, P_x, in_features, out_features, dim=-1, bias=True, device=torch.device('cpu'), dtype=torch.float32):
        ...
        if self.P_root.active:
            self.W = nn.Parameter(self.scale*torch.rand(out_features, in_features, device=device, dtype=dtype))
            self.b = nn.Parameter(1/self.out_features*torch.rand(*self.b_shape, device=device, dtype=dtype))
...
class ParallelFNOBlock(nn.Module):
    ...
    def __init__(self, P_x, in_shape, modes, device=torch.device('cpu'), dtype=torch.float32):
        ...
        # Setup weights
        self.scale = 1/np.sqrt(self.width*self.width)
        ...
        self.linear = BroadcastedLinear(self.P_x, self.width, self.width, dim=1, device=device, dtype=dtype)
...
if __name__ == '__main__':
    ...
    with cupy.cuda.Device(device_ordinal):
        layer = ParallelFNO(P_x, in_shape, nt, width, modes, num_blocks=4, device=x.device, dtype=x.dtype)
        y = layer(x)

    for name, p in layer.named_parameters():
        print(f'rank = {P_x.rank}, {name} -> {p.shape}')
