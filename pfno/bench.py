...
from utils import profile_gpu_memory, get_env
...
def bench(input_shape, partition_shape, width, modes, nt, num_gpus, benchmark_type, output_dir=Path('.')):

    P_world, P_x, P_0 = create_standard_partitions(partition_shape)
    use_cuda, cuda_aware, device_ordinal, device, ctx = get_env(P_x, num_gpus)
    ...
    bench_gpu_mem = True if use_cuda else False
    if bench_gpu_mem:
